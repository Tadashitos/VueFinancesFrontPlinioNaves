...
const formatError = message => {
    //mensagem de erro: 'GraphQL error: Credenciais inválidas'
    const messageSplit = message.split(':') //messageSplit = ['GraphQL error', ' Credenciais inválidas']
    return messageSplit[messageSplit.length - 1].trim()
}

/*
[1,2,5,3].reduce((somaAtual, numero) => {
        ...
        case 'bar':
            tooltips = {
                callbacks: {
                    title() { },
                    label(tooltip, data) {
                        return data.datasets[tooltip.dataIndex].label
                    }
                }
            }
            break
...
const generateChartData = ({ items, keyToGroup, keyOfValue, aliases, type, backgroundColors }) => {
    ...
    for(let key in grouped) {
        ...
        grouped[key].reduce((acc, item) => acc + item[keyOfValue], 0) // keyOfValue = amount
    }
...
const generateChartConfigs = (options) => {
    const { type } = options
    const data = generateChartData(options)
    const chartOptions = generateChartOptions(type)
